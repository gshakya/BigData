...
//			mapper[i].mapValues();
	...
	//System.out.println(partitionPairs);
		...
		for (int i = 0; i < reducer.length; i++) {
			...
			System.out.println("\n_____________Reducer " + i
					+ " Output_____________\n");
			List<StripedElement<Integer,HashMap<Integer,Float>>> reducerOutput = reducer
...
//		System.out.println(shuffledKeysList);
	...
	private void printKeyValue(List<StripedElement<Integer,HashMap<Integer,Float>>> reducerOutput) {
		// TODO Auto-generated method stub
		if(reducerOutput !=null)
		{
			for(StripedElement<Integer,HashMap<Integer,Float>> item : reducerOutput)
			{
